...
#endif
  const uint8_t size = msg->trans->size_of(msg, /* msg header overhead */4+4+1+1+1+1+1+1+1+1+1+2+2);
  if (msg->trans->check_available_space(msg, _FD_ADDR, size)) {
    msg->trans->count_bytes(msg, size);
    msg->trans->start_message(msg, _FD, /* msg header overhead */4+4+1+1+1+1+1+1+1+1+1+2+2);
...
static inline void pprz_msg_send_ROTORCRAFT_STATUS(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, uint32_t *_link_imu_nb_err, uint8_t *_motor_nb_err, uint8_t *_rc_status, uint8_t *_frame_rate, uint8_t *_gps_status, uint8_t *_ap_mode, uint8_t *_ap_in_flight, uint8_t *_ap_motors_on, uint8_t *_ap_h_mode, uint8_t *_ap_v_mode, uint16_t *_vsupply, uint16_t *_cpu_time) {
    ...
    msg.receiver_id = 0;
    msg.component_id = 0;
    pprzlink_msg_v2_send_ROTORCRAFT_STATUS(&msg,_link_imu_nb_err,_motor_nb_err,_rc_status,_frame_rate,_gps_status,_ap_mode,_ap_in_flight,_ap_motors_on,_ap_h_mode,_ap_v_mode,_vsupply,_cpu_time);
}
...
/** Getter for field vsupply in message ROTORCRAFT_STATUS
  ...
  * @return 
  */
static inline uint16_t pprzlink_get_DL_ROTORCRAFT_STATUS_vsupply(uint8_t * _payload __attribute__((unused)))
{
    return _PPRZ_VAL_uint16_t(_payload, 17);
}
