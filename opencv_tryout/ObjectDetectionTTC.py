...
def filterMatches(matches, prevKeypoints, currKeypoints, threshold=30):
  ...
  filteredMatches = list(filter(lambda m: m.distance < threshold and currKeypoints[m.trainIdx].size > prevKeypoints[m.queryIdx].size, matches))
...
def matchTemplates(matches, prevKeypoints, currKeypoints, prevFrame, currFrame):
  ...
  for m in matches:
    ...
    for s in np.arange(1., 2.1, 0.1):
      ...
      currTemplate = createTemplate(prevKeypoints[m.queryIdx], currKeypoints[m.trainIdx], currFrame, scale=1.2/9*20*s)
      ...
      templateMatch = np.sqrt(((currTemplate[:rows, :cols] - scaledPrevTemplate[:rows, :cols])**2 / (s*s)).sum())
    ...
    comparisonTemplates = np.zeros((max(prevTemplate.shape[0], bestCurrTemplate.shape[0]), prevTemplate.shape[1] + bestCurrTemplate.shape[1]))
...
def draw(prevFrame, prevKeypoints, currFrame=None, currKeypoints=None, matches=None, method='matches', mColor=(0, 255, 0), sColor=(0, 0, 255), flag=4):
  """
  ...
  if method is 'keypoints':
    ...
    drawing = cv2.drawMatches(prevFrame, prevKeypoints, currFrame, currKeypoints, matches, None, mColor, sColor, None, flag)
...
if __name__ == '__main__':
  ...
  while True:
    ...
    detector, prevKeypoints, prevDescriptors, currKeypoints, currDescriptors = getKeypoints(prevFrame_gray, currFrame_gray)
